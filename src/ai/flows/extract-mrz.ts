...
1.  **Источник данных**: Основные поля (фамилия, имя, номер документа, дата рождения, гражданство, пол, тип документа, страна выдачи, персональный номер и т.д.) ДОЛЖНЫ быть извлечены из строки MRZ в конце текста. Поля 'dateOfIssue', 'placeOfBirth' и 'authority' должны быть найдены в остальной (визуальной) части текста.
2.  **Анализ и форматирование**:
    ...
    *   **Имена ('surname' & 'givenName')**: Извлеките ИСКЛЮЧИТЕЛЬНО из строки MRZ. Замените символы-заполнители '<' на пробелы.
3.  **Формат вывода**:
